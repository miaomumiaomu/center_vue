<template>
  <!-- 编辑车辆库弹层 S -->
    <el-dialog :title="id? readonly ? '车辆详情' : '编辑车辆':'新增车辆'" class="form-layer" :visible.sync="isAddFormDialog" width="700px" append-to-body>
      <el-form :rules="rules" :model="form" ref="form" label-width="100px" :style="waterBg" :disabled="readonly">
      ...
      <div class="heading">
        ...
        <span style="float: right">
          <el-button class="txt-btn" size="mini" type="text" icon="el-icon-plus" :disabled="readonly" @click="onItemEditadd()">选择人员库成员</el-button>
          <el-button class="txt-btn" size="mini" type="text" icon="el-icon-plus" :disabled="readonly" @click="onItemEdit()">手动添加</el-button>
        </span>
      </div>
      ...
      <div class="heading-house">
        <span>房屋信息</span>
        <span style="float: right">
          <el-button class="txt-btn" size="mini" type="text" v-if="form.id" @click="carHistoryOpen(form.id)">变更历史</el-button>
        </span>
      </div>
      <div>
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="房屋" style="margin: 0;">
                  <el-input v-model="housesForm[index].fullName" :title="housesForm[index].fullName" placeholder="实有房屋" @focus="addressDialogOpen(index)" readonly :disabled="readonly"/>
                </el-form-item>
              </el-col>
              <!-- <el-col :span="12">
                <el-form-item label="房屋" style="margin: 0;">
                  ...
                  <el-input v-model="housesForm[index].houseName" :title="housesForm[index].houseName" placeholder="请先选择地址" v-else readonly :disabled="readonly"/>
                </el-form-item>
              </el-col> -->
            </el-row>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini" type="primary" @click="onUpdate()" v-if="!readonly">确 定</el-button>
        <el-button size="mini" @click="onCancle()" v-if="!readonly">取 消</el-button>
        <el-button size="mini" @click="onCancle()" v-if="readonly">关 闭</el-button>
      </div>
      <!-- 表单详情 -->
      <person-select :is-open.sync="isFormDialogadd" v-on:choseMan="choseMan" ref="SelectForm" title="选择车主"/>

      <label-form ref="labelForm" :id.sync="personId" :is-open.sync="isFormDialog" v-on:choseMan="choseMan"/>
