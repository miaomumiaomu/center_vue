<template>
  <div class="librarie-select">
    <div class="librarie-box" @click="isLibrarieDialog = true">
      ...
      <div class="placeholder" v-else>请选择人员标签</div>
    </div>

    <el-dialog title="选择人员标签" class="librarie-layer body0" :visible.sync="isLibrarieDialog" width="560px" append-to-body>
      <el-row>
        <el-col :span="12" class="left-box">
          <el-tree class="filter-menu" show-checkbox :data="librarieData" @check="onTreeChange" node-key="id" :expand-on-click-node="false" ref="tree" default-expand-all>
            <div class="menu-item" slot-scope="{data}">
        ...
        <el-col :span="12" class="right-box">
          <div class="title">已选择了{{librarieArr.length}}个人员标签</div>
  ...
  methods: {
    async init() {
      ...
      await getTagList().then(res => {
        ...
        this.librarieData = _data;
      });
    },
    updateTree() {
      this.$nextTick(() => {
        if (this.$refs.tree) {
          console.log('updateData', this.updateData.map(item => item.id))
