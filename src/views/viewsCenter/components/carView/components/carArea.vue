<template>
  <div class="car-distribution">
		<div class="top">
			<div class="title">今日车辆归属地</div>
		</div>
		<div class="table">
      <bar-chart :data="data" height="20vh" :colorIndex="1"/>
		</div>
	</div>
</template>
<script>
import{ getCarPlaceOwner } from '@/api/viewsCenter/carView'
export default {
	data() {
		return {
			data: [],
	...
	methods: {
		getCarPlaceOwner() {
			getCarPlaceOwner({ ...this.code }).then(res => {
				this.data = res.data
			})
		}
	}
