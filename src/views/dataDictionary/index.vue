  ...
  <div class="main">
      ...
      <el-col :span="19" style="height:100%">
        <div class="app-container">
          <div class="filter-container">
            <el-form :model="listDictValueQuery" ref="listForm" :inline="true" class="form" label-width="60px">
              <el-form-item prop="_filter_like_code" label="编码">
                <el-input type="text" size="mini" v-model="listDictValueQuery._filter_like_code" @keyup.enter.native="onSearch" clearable></el-input>
              </el-form-item>
              <el-form-item prop="_filter_like_label" label="名称">
                <el-input type="text" size="mini" v-model="listDictValueQuery._filter_like_label" @keyup.enter.native="onSearch" clearable></el-input>
              </el-form-item>
              <el-form-item>
          ...
          <div class="common-tool">
            ...
            <div class="right">
              ...
              <el-button class="txt-btn" style="margin-left:0px" size="mini" icon="el-icon-delete" type="text" @click="onItemDelete">删除</el-button>
            </div>
          </div>
          <el-table :data="dictValueTableData.rows" highlight-current-row fit style="width: 100%" height="800px" @selection-change="onListChange">
            ...
            <el-table-column align="center" label="字典值" prop="value" />
            ...
            <el-table-column align="center" label="中文显示" prop="label" />
          ...
          <el-pagination @size-change="onSizeChange" @current-change="onCurrentChange" :current-page.sync="listDictValueQuery.pageNo" :page-sizes="[15,30,50]" :page-size="listDictValueQuery.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="dictValueTableData.total" />
        </div>
      </el-col>
    </el-row>
    ...
    <el-dialog :visible.sync="isFormDialog" :title="dictTypeForm.id?'编辑字典类型':'新增字典类型'" width="500px">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini" type="primary" @click="onSubmitType">保存</el-button>
    ...
    <el-dialog :title="dictValueForm.id?'编辑字典值':'新增字典值'" :visible.sync="dictValueDialogVisible" width="500px">
      <el-form :model="dictValueForm" :rules="dictValueFormRules" ref="dictValueForm" label-width="80px">
        <el-form-item label="编码值" prop="code">
          <el-input v-model="dictValueForm.code" placeholder="请输入编码">
            <template slot="prepend">{{currentDictType.code+'_'}}</template>
          </el-input>
        </el-form-item>
        <el-form-item label="字典值" prop="value">
          <el-input v-model="dictValueForm.value" placeholder="请输入字典值" />
        </el-form-item>
        <!-- <el-form-item label="英文显示" prop="labelEnUs">
          <el-input v-model="dictValueForm.labelEnUs" placeholder="请输入英文显示" />
        </el-form-item> -->
        <el-form-item label="名称" prop="label">
          <el-input v-model="dictValueForm.label" placeholder="请输入名称" />
        </el-form-item>
        <el-form-item label="排序" prop="sortNo">
          <el-input v-model="dictValueForm.sortNo" placeholder="请输入排序" />
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" type="primary" @click="onSubmitValue('dictValueForm')">确 定</el-button>
        <el-button size="mini" @click="dictValueDialogVisible = false;">取 消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      listDictValueQuery: {
        ...
        _filter_like_label: null,
        _filter_like_typeId: null
      },
      dictValueForm: {
        code: undefined,
        value: undefined,
      ...
      dictValueFormRules: {
        ...
        // labelEnUs: [
        //   { required: true, message: "请输入英文显示", trigger: "blur" },
        //   { min: 1, max: 20, message: "长度在 1 到 20 个字符", trigger: "blur" }
        // ],
        label: [
  ...
  methods: {
    ...
    getNodeData(data) {
      ...
      this.listDictValueQuery._filter_like_typeId = data.id;
    ...
    onCommand(command, data) {
      ...
      if (command == "del") {
        this.$confirm(`确定删除【${data.label}】？`, "提示", {
          ...
          deleteDictType({ids: [data.id]}).then(res => {
      ...
      this.$refs.dictTypeForm.validate(valid => {
        ...
        // } else {
          postDictType(this.dictTypeForm).then(res => {
            ...
            this.$notify({
              title: isSuccess ? "成功" : "失败",
              message: isSuccess ? `${this.dictTypeForm.id ? '编辑' : '添加'}成功` : res.message,
              type: isSuccess ? "success" : "error",
              duration: 2000
            });
    ...
    onSizeChange(val) {
      this.listDictValueQuery.pageSize = val;
      getDictValueList(this.listDictValueQuery).then(response => {
        this.dictValueTableData = response.data;
      });
    },
    onCurrentChange(val) {
      this.listDictValueQuery.pageNo = val;
      getDictValueList(this.listDictValueQuery).then(response => {
        this.dictValueTableData = response.data;
      });
    },
    ...
    onSearch() {
      this.listDictValueQuery.pageNo = 1;
      this.listDictValueQuery._filter_like_typeId = this.currentDictTypeId;
      getDictValueList(this.listDictValueQuery).then(response => {
        this.dictValueTableData = response.data;
      });
    },
      ...
      set[formName].validate(valid => {
        ...
        // } else {
          ...
          postDictValue(this.dictValueForm).then(() => {
            ...
            this.$notify({
              title: "成功",
              message: `${this.dictValueForm.id ? '编辑' : '添加'}成功`,
              type: "success",
              duration: 2000
            });
            this.dictValueDialogVisible = false;
